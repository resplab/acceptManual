...
library(htmltools)
rawHTML <- paste(readLines("excel-input1.html"), collapse="\n")
HTML(rawHTML)
```
...
```{r, eval=TRUE, echo=FALSE}
library(htmltools)
rawHTML <- paste(readLines("excel-input2.html"), collapse="\n")
HTML(rawHTML)
```
...
library(htmltools)
source("FormatCode.R")
htmlLines = readLines("install.html")
...
library(htmltools)
source("FormatCode.R")
htmlLines = readLines("get-default-input.html")
newLines = addBreaks(htmlLines)
rawHTML <- paste(newLines, collapse="\n")
HTML(rawHTML)
```
...
library(htmltools)
source("FormatCode.R")
htmlLines = readLines("post-prism-default.html")
newLines = addBreaks(htmlLines)
rawHTML <- paste(newLines, collapse="\n")
HTML(rawHTML)
```
...
```{r, eval=TRUE, echo=FALSE}
htmlLines = readLines("post-prism.html")
newLines = addBreaks(htmlLines)
rawHTML <- paste(newLines, collapse="\n")
HTML(rawHTML)
```
